 ...
 * [16.3.2b] 산울동 탁구장 안내 미종료 문자열(컴파일 에러) 수정
 * ─────────────────────────────────────────────
 */
...
function createBungae(sender, args) {
    ...
    // "세종 벙개 취소" 가 장소=취소 로 새 벙개 만들어지던 버그 차단
    if (!args || args === "취소" || args === "종료" || args === "삭제" || args === "닫기") {
        return cancelBungae(sender);
    }
    if (args === "현황" || args === "상태" || args === "확인") {
        return showBungaeStatus();
    }
    if (args.indexOf("취소") === 0 || args.indexOf("종료") === 0) {
        return cancelBungae(sender);
    }

    var cur = loadJSON(FILE_BUNGGAE, { host: "", members: [], wait: [], max: 8 });
    if (cur && cur.host && cur.created === getTodayKey()) {
        return "⚠️ 이미 진행 중인 벙개가 있어요!\n\n" +
               showBungaeStatus() + "\n\n" +
               "💡 새로 열려면 먼저: 세종 벙개취소 (벙주/관리자)\n" +
               "💡 사람 추가: 세종 추가참가 [닉네임]";
    }

    var parts = args ? args.split(/\s+/) : [];
    var banned = { "취소": 1, "종료": 1, "현황": 1, "참가": 1, "추가": 1, "삭제": 1 };
...
function joinBungae(sender) {
    ...
    updateJSON(FILE_BUNGGAE, { host: "", members: [], wait: [], max: 8 }, function(data) {
        if (!data.host) {
            outcome = "❌ 진행 중인 벙개가 없어요.\n💡 열기: 세종 벙개 [장소] [시간]";
            return undefined;
        }
        data.members = uniqNames(data.members || []);
        data.wait = uniqNames(data.wait || []);
        if (!data.max || data.max < 2) data.max = 8;

        if (data.members.indexOf(name) >= 0) {
            outcome = "ℹ️ 이미 참가 중이에요.\n👥 " + data.members.length + "/" + data.max + "명";
            return undefined;
        }
        if (data.wait.indexOf(name) >= 0) {
            outcome = "ℹ️ 이미 대기 중이에요.\n대기 순번: " + (data.wait.indexOf(name) + 1);
            return undefined;
        }
...
/** 벙주/관리자가 다른 사람 넣기: 세종 추가참가 옥사마 */
function addMemberToBungae(sender, nickArgs) {
    var nick = String(nickArgs || "").replace(/\s+/g, " ").trim();
    if (!nick) {
        return "💡 사용법: 세종 추가참가 [닉네임]\n예: 세종 추가참가 옥사마\n(벙주 또는 관리자만)";
    }
    var name = getShortName(nick) || nick;
    var outcome = "";
    ...
    updateJSON(FILE_BUNGGAE, { host: "", members: [], wait: [], max: 8 }, function(data) {
        ...
        var isHost = (data.host === short) ||
            (data.hostFull && String(data.hostFull).indexOf(short) >= 0) ||
            (String(sender).indexOf(data.host) >= 0);
        if (!isAdmin(sender) && !isHost) {
            outcome = "🚫 추가참가는 벙주 또는 관리자만 가능해요.\n일반 참가: 세종 참가";
            return undefined;
        }
        ...
        if (data.members.indexOf(name) >= 0) {
            outcome = "ℹ️ " + name + "님은 이미 참가 중이에요.\n👥 " + data.members.length + "/" + data.max + "명";
            return undefined;
        }

        var widx = data.wait.indexOf(name);
        if (widx >= 0) data.wait.splice(widx, 1);

        if (data.members.length >= data.max) {
            data.wait.push(name);
            outcome = "⏸️ 정원 가득 → " + name + "님 대기 등록\n대기 " + data.wait.length + " · 정원 " + data.max;
            return data;
        }

        data.members.push(name);
        outcome = "✅ " + name + "님 추가 참가 완료!\n\n👥 " + data.members.length + "/" + data.max + "명\n(등록: " + short + ")";
        return data;
    });

    if (outcome.indexOf("✅") === 0) {
        incrementStat("bunggae_joins");
        addAwardLog("벙개참가", name, "추가 참가 by " + getShortName(sender));
    }
    return outcome;
}
...
function leaveBungae(sender) {
    ...
    updateJSON(FILE_BUNGGAE, { host: "", members: [], wait: [], max: 8 }, function(data) {
        ...
        if (idx >= 0) {
            ...
            if (data.host === name && data.members.length > 0) {
                data.host = data.members[0];
            }
            if (data.wait && data.wait.length > 0 && data.members.length < (data.max || 8)) {
    ...
    if (outcome === "leave_empty") {
        addAwardLog("벙개취소", sender, "마지막 참가 취소 → 벙개 종료");
        return "✅ 참가 취소 완료\n(남은 인원 없음 → 벙개 종료)";
    }
    if (outcome === "leave_member") {
        addAwardLog("벙개취소", sender, "벙개 참가 취소");
        if (promoted) {
            addAwardLog("벙개참가", promoted, "대기자 자동 승급");
            return "✅ 참가 취소 완료\n\n⏫ 대기자 " + promoted + "님 자동 참가";
        }
        return "✅ 참가 취소 완료";
    }
...
function showBungaeStatus() {
    ...
    var cnt = data.members.length;
    var dateStr = data.dateLabel || todayDateLabel();
    ...
    if (cnt === 0) msg += "(없음)\n";
        ...
        for (var i = 0; i < data.members.length; i++) {
            var mark = (data.members[i] === data.host) ? " 👑" : "";
            msg += (i + 1) + ". " + data.members[i] + mark + "\n";
        }
    }
    if (data.wait && data.wait.length > 0) {
        msg += "\n⏸️ 대기자 (" + data.wait.length + "명)\n";
...
function cancelBungae(sender) {
    ...
    updateJSON(FILE_BUNGGAE, { host: "", members: [] }, function(data) {
        ...
        var isHost = (data.host === short) ||
            (data.hostFull && String(data.hostFull).indexOf(short) >= 0) ||
            (String(sender).indexOf(data.host) >= 0);
        if (!isAdmin(sender) && !isHost) { snapshot = "deny"; return undefined; }

        allowed = true;
        snapshot = {
            place: data.place, time: data.time,
            members: uniqNames(data.members || []).slice()
        };
        return { host: "", members: [], wait: [], max: 8 };
    });
